...
function reportToHtml(report) {
  ...
  if (report.publication) {
    ...
    for (const r of report.publication.actorRollup ?? []) {
      const flag = r.isUnknown ? "⚠️ " : r.whitelisted ? "✅ " : "";
  ...
  if (report.completeness) {
    ...
    for (const i of report.completeness.issues ?? []) {
      ...
      lines.push(`<li>${sev} ${escape(game.i18n.format(`${i.i18nKey}.Title`, i.params || {}))}</li>`);
    }
    lines.push(`</ul>`);
  }
  if (report.prerequisites) {
    ...
    for (const i of report.prerequisites.issues ?? []) {
      const sev = i.evaluation === "fail" ? "🔴" : "🔵";
      lines.push(`<li>${sev} <b>${escape(i.featName)}</b> — ${escape(i.requirement)}</li>`);
    }
    lines.push(`</ul>`);
  }
  return lines.join("\n");
